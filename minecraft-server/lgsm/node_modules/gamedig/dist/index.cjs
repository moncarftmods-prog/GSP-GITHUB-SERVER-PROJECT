...
var DnsResolver = class {
  ...
  async resolve(host, ipFamily, srvRecordPrefix) {
    ...
    if ((0, import_node_net2.isIP)(host)) {
...
// protocols/core.js
var import_telnet_client = require("telnet-client");
var uid = 0;
var Core = class extends import_node_events3.EventEmitter {
  constructor() {
    ...
    this.shortestRTT = 0;
    this.usedTcp = false;
    this.telnetClient = new import_telnet_client.Telnet();
  }
...
var buildandshoot = class extends Core {
  async run(state) {
    ...
    const bluePlayers = ((_a = request.players) == null ? void 0 : _a.blue) || [];
    const greenPlayers = ((_b = request.players) == null ? void 0 : _b.green) || [];
    ...
    state.maxplayers = request.maxPlayers || ((_c = request.players) == null ? void 0 : _c.maxPlayers);
    state.players = [];
...
var farmingsimulator = class extends Core {
  async run(state) {
    ...
    const isValidXML = import_fast_xml_parser.XMLValidator.validate(request);
    if (!isValidXML) {
    ...
    const mods = ((_a = serverInfo == null ? void 0 : serverInfo.Mods) == null ? void 0 : _a.Mod) || [];
    ...
    if ("" in state.raw.playerTeamInfo) {
      for (const playerInfo of state.raw.playerTeamInfo[""]) {
        ...
        for (const from of Object.keys(playerInfo)) {
          ...
          if (key === "player")
            key = "name";
          if (["score", "ping", "team", "deaths", "pid"].includes(key))
            value = parseInt(value);
          player[key] = value;
        }
        state.players.push(player);
...
var minetest = class extends Core {
  ...
  async run(state) {
    ...
    const serverInfo = servers.list.find(
      (server) => server.address === this.options.address && server.port === this.options.port
    );
    if (serverInfo == null) {
      throw new Error("Server not found in master server list");
    }
  ...
  async run(state) {
    {
      ...
      switch (version) {
        ...
        case 5:
          ...
          state.raw.gamescript_name = reader.string();
        case 4:
      ...
      if (version < 7) {
        const DAY_TICKS = 74;
        state.raw.ticks_playing = (new Date(state.raw.date_current).getTime() - new Date(state.raw.date_start).getTime()) / 1e3 / 3600 / 24 * DAY_TICKS + 1280;
      }
    }
  }
...
// protocols/sdtd.js
var playerRegex = /(?<=id=\d+,\s*)(?<name>\S[^,]*)(?=,)/;
var gameVersionsRegex = /V \d+\.\d+(?: \(b\d+\))?/g;
var modRegex = /^Mod\s+([^:]+):\s*([\d.]+)$/;
var dateTimeRegex = /Day\s+(\d+),\s*(\d{2}:\d{2})/;
var sanitizeTelnetResponse = (response) => {
  return response.split(/\r?\n/).map((l) => l.replace(/\r$/, "").trim()).filter((l) => l.length > 0);
};
var sdtd = class extends valve {
  async run(state) {
    await super.run(state);
  ...
  async telnetMoreData(state) {
    ...
    for (const possibleMod of versions.slice(1)) {
      const match2 = possibleMod.match(modRegex);
      if (match2) {
        mods.push({
          name: match2[1],
          version: match2[2]
...
var scpsl = class extends Core {
  ...
  async run(state) {
    ...
    const servers = (request == null ? void 0 : request.Servers) || [];
    if (!servers.length) {
      throw new Error("No servers provided.");
    }
    const server = servers.find((server2) => server2.ID.toString() === serverId);
    if (!server) {
